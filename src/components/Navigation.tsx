...
const Navigation: React.FC<Props> = ({ dragNavPanelKey, setDragNavPanelKey }) => {
  ...
  return (
    <nav style={{ width: 90, background: '#232b3e', padding: '0.5rem 0.25rem', borderRight: '1px solid #3e4a6b' }}>
        ...
        {panelList.map(panel => (
          ...
          >
            <div style={{ marginBottom: 4 }}>
              {panel.key === 'fruitbook' ? <TermsIcon /> : panel.key === 'fruitview' ? <FruitViewIcon /> : <AboutIcon />}
            </div>
            <div>{panel.title}</div>
          </li>
        ))}
