...
function App() {
  ...
  const onMainDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    setOpenPanels([...openPanels, { id, key: panelDef.key, title: panelDef.title, content: panelDef.content, x, y, width, height }]);
    setDragNavPanelKey(null);
  };

  return (
    <div className={`app-root theme-${theme}`} style={{ display: 'flex', height: '100vh' }}>
      {navOpen && <Navigation dragNavPanelKey={dragNavPanelKey} setDragNavPanelKey={setDragNavPanelKey} />}
      <MainWorkspace onDrop={onMainDrop} onDragOver={e => e.preventDefault()} onGridDropInfo={handleGridDropInfo} gridRows={GRID_ROWS} gridCols={GRID_COLS}>
        <main style={{ flex: 1, position: 'relative', background: '#232b3e', overflow: 'hidden' }}>
          ...
          {openPanels.length === 0 ? (
            <div style={{ color: '#888', textAlign: 'center', marginTop: '2rem' }}>No panels open.<br />Drag one from the navigation bar.</div>
          ) : (
            openPanels.map(panel => (
              <ResizableDraggablePanel
                ...
                onMove={(dx, dy) => setOpenPanels(panels => panels.map(p => (p.id === panel.id ? { ...p, x: p.x + dx, y: p.y + dy } : p)))}
                onResize={(dw, dh) => setOpenPanels(panels => panels.map(p => (p.id === panel.id ? { ...p, width: Math.max(200, p.width + dw), height: Math.max(100, p.height + dh) } : p)))}
              />
            ))
          )}
        </main>
